...
if(isset($_POST["name"]) && isset($_POST["phone"])){
  ...
  $q="SELECT * FROM `users` WHERE user_name='$name' AND phone_number='$phone'";
  ...
  if($rq=mysqli_query($db,$q)){
    ...
    if(mysqli_num_rows($rq)==1){
      ...
      $q="SELECT * FROM `users` WHERE phone_number='$phone'";
      if($rq=mysqli_query($db,$q)){
        if(mysqli_num_rows($rq)==1){
          echo "<script>alert($phone+' is already taken by another person')</script>";
        }else{

          $q="INSERT INTO `users`(`user_name`, `phone_number`) VALUES ('$name','$phone')";
          if($rq=mysqli_query($db,$q)){
            $q="SELECT * FROM `users` WHERE user_name='$name' AND phone_number='$phone'";
            if($rq=mysqli_query($db,$q)){
              if(mysqli_num_rows($rq)==1){
